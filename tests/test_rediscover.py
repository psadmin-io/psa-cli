...
class TestRediscoverAfterApply:
    """Test --rediscover triggers hub push after successful puppet apply."""
    ...
    @patch("psa.commands.discover._push_to_hub")
    def test_push_called_with_rediscover(self, mock_push, mock_run, mock_get_config, tmp_path):
        ...
        hub_cfg = HubConfig(url="http://hub:8002", environment_id="env1")
        cfg = PsaConfig(ps_cfg_home=tmp_path, hub=hub_cfg)
    ...
    def test_hub_not_configured_warns(self, mock_run, mock_get_config, tmp_path):
        """--rediscover with no hub config prints warning, does not crash."""
        dpk_path = self._build_dpk_tree(tmp_path)

        # Hub NOT configured (no url)
        cfg = PsaConfig(ps_cfg_home=tmp_path, hub=HubConfig())
        mock_get_config.return_value = cfg
